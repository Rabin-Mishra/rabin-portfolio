...
export function AboutHero({ config }: AboutHeroProps) {
  const ownerName = config?.ownerName ?? "Rabin Mishra";
  const tagline = config?.tagline ?? "IT Engineer | Aspiring DevOps";
  const location = config?.location ?? "Kathmandu, Nepal";
  const profileImageUrl = config?.profileImage ? urlForImage(config.profileImage).url() : "/profile.jpeg";
      ...
      <div className="container mx-auto px-4 md:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-4 flex flex-col items-center text-center space-y-6">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-textPrimary mb-2">{ownerName}</h1>
          ...
          <div className="lg:col-span-8">
            ...
            <div className="space-y-6 text-lg text-textMuted leading-relaxed">
              {config?.longBio && config.longBio.length > 0 ? (
                <PortableTextRenderer value={config.longBio} />
              ) : (
                <>
                  <p>
